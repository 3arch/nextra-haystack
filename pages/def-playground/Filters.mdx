...
Filters are a simple declarative language used to query Haystack data.  A filter is a predicate function that takes a dict and returns true/false (match/no match).  Some filter constructs require the use of [defs](Defs) in which case the filter must be evaluated in the context of a [namespace](Namespaces).
...
The scalars to compare against are encoded using [Zinc](Zinc) encoding (with a couple of exceptions noted below).
...
## <a name="arrow"></a>Arrow Operator

You use the `->` operator to dereference a tag that has a Ref value.  For example, if your `equip` rec has a `siteRef` tag that references the site, you can query for equip in a given city such as:
...
Consider an example with a VAV that has an `airRef` configured as a list of two refs:
...
See [Zinc grammar](Zinc#grammar) for productions reused from Zinc.  Bools are encoded as "true" or "false" (Zinc encodes as "T" or "F").

...
Libs, or libraries, are the fundamental unit of modularity in Project Haystack. Every def must be defined inside a parent lib.  The normalized representation of every def has a [lib](lib) tag with a symbolic link to its parent library. However, the `lib` tag itself is never declared explicitly. Rather, it is always implied during load time as part of [normalization](Normalization).

Libs are defined as a zip file with one or more [Trio](Trio) files under the "lib/" directory.  There must be a file named "lib/lib.trio" which specifies exactly one dict for the lib's metadata.  The lib metadata should declare the following tags:

* [def](def): must be a feature key under the [lib](lib) feature
* [doc](doc): must provide a description of the library
* [version](version): identifies the version as series of integers separated by dot
* [baseUri](baseUri): an absolute URI used to map symbols to URIs (used by [RDF](Rdf))
* [depends](depends): a list symbolic names to libs specifying dependencies
...
Here is an example for [lib:phIoT](lib:phIoT):
...
## <a name="phLibs"></a>Project Haystack Libs

Project Haystack currently defines four standardized libs:

* [lib:ph](lib:ph): core definitions
* [lib:phScience](lib:phScience): definitions related to scientific phenomenon and quantities
* [lib:phIoT](lib:phIoT): definitions related to domain of Internet of Things and the built environment
* [lib:phIct](lib:phIct): definitions related to Information and Communication Technology


Future Project Haystack lib names will be always be prefixed with "ph".

## <a name="lib"></a>Lib Namespaces

Libs import external names into their local namespace via the [depends](depends) tag.  All libs must include `lib:ph` (with the exception of `lib:ph` itself). The namespace of all included defs and locally defined defs in a library is called the *lib namespace*.

Only names found in the lib's namespace may be used for definition tag names or symbol values.  For example, if a def needs to subtype from the [equip](equip) tag, then its lib must declare an include on [lib:phIoT](lib:phIoT) to bring `equip` into the namespace.  It is invalid to use a tag name or symbol value in a def that is not found in the lib's namespace.

## <a name="proj"></a>Project Namespaces
...
## <a name="ns"></a>Symbolic Namespace

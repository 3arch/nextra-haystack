...
* [Ontology](#ontology): a standard way to model common concepts such as buildings, equipment, and sensors
...
Project Haystack is focused on the semantic data model for the [IoT](https://en.wikipedia.org/wiki/Internet_of_things) and the [built environment](https://en.wikipedia.org/wiki/Built_environment). We specify an ontology to standardize modeling of common concepts in this domain including:
...
* [id](id): primary key for this data item (within its data set)
* [dis](dis): human display name for the data item
* [site](site): marker tag that tells us this data represents a building
* [area](area): square footage of the building
* [geoAddr](geoAddr): street address of the building
* [tz](tz): timezone to use for this building
* [weatherStationRef](weatherStationRef): relationship for which weather station to use for this building


These concepts are explored further in the [Ontology](Ontology) chapter and the individual chapters for specific systems.
...
Defs are themselves just a dict that uses a standard set of *meta tags* for their definition.  This means defs are also normal Haystack data. For example, the def for the [floor](floor) tag is itself a dict of the following tags:
...
Defs, and the mechanisms used to construct the ontology, are introduced in detail in the [Defs](Defs) chapter.

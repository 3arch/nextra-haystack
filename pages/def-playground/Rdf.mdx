...
Ontological relationships are conveyed in the Haystack information model using [defs](Defs). Haystack provides methods to query information about the relationships between defs, and to make inferences about these relationships. These defs are stored in a format, viz., Trio, that cannot be consumed by traditional semantic tools.

In order to make the Haystack ontology more accessible to users more familiar with standard tools for the semantic web, we define a mapping from Haystack defs to [RDF](https://www.w3.org/RDF/). RDF is considered the "lingua franca" of the semantic web, so it is highly desirable to have a well-defined set of rules for exporting Haystack defs to RDF. We adopt the following high-level goals in defining this mapping:

* Export defs as RDF statements in [Turtle](https://www.w3.org/TR/turtle/) format.
* Generate [RDFS](https://www.w3.org/TR/rdf-schema/) statements to add semantic meaning that is equivalent to the def.
* Use [OWL](https://www.w3.org/TR/owl2-primer/) statements where they serve to increase the expressivity and usability of the model.


The rest of this document outlines a strategy for mapping Haystack defs to RDF.

## <a name="genRules"></a>General Mapping Rules
...
* [doc](doc) tags are mapped to `rdfs:comment` predicates.
* Values of the [is](is) list become distinct `predicate/object` pairs


RDF requires that every resource be uniquely identified by an [IRI](https://en.wikipedia.org/wiki/Internationalized_Resource_Identifier). Every Def is a logical resource, so we need to map def symbols to IRIs. To construct an IRI from a symbol:
...
For example, [site](site) is defined in the `phIoT` library. That library has the following def:
...
## <a name="markers"></a>Marker Tags

Defs for marker tags are subtypes of [marker](marker) via the [is](is) tag. See the section on [Subtyping](Subtyping) for details.

Marker tag defs become instances of an `owl:Class`.

The supertype tree defined by [is](is) maps to a set of `rdfs:subClassOf` statements. This establishes a subtyping relationship in the ontology.

Using these rules, the [site](site) def now becomes
...
Haystack defines a number of data types (e.g. Str, Bool, DateTime). All data types in Haystack are subtypes of the [val](val) def. Scalar data types ([scalar](scalar)) are further refined as subtypes of [val](val). This section describes the rules for defining scalar data types in the RDF export.

Only *direct* subtypes of [scalar](scalar) are defined as data types. They are declared as instances of the `owl:DatatypeProperty` class.

**Exception**: marker tags are not defined as data types since they convey subtyping information (see [Marker Tags](#markers) section).
...
## <a name="valTags"></a>Value Tags

A value tag def is any tag def that is **not** a subtype of [marker](marker).

If the tag def is a subtype of [ref](ref) or is a [choice](choice), then declare the def to be an instance of `owl:ObjectProperty`. All other value tags are declared to be an instance of `owl:DatatypeProperty`.

If the tag def (or one of its [defx](defx) extensions) declares a [tagOn](tagOn) association, then specify the `rdfs:domain` to be all referent entities.

If the tag def is a [ref](ref) or [choice](choice), then specify the `rdfs:range` to the value of the [of](of) tag on the def.

If the def is a basic [Tag Def](Defs#tags) (e.g. [tz](tz)), then specify the `rdfs:range` to be the appropriate Haystack datatype (see [Datatypes](#datatypes) section).
...
## <a name="instances"></a>Exporting Instances

To this point we have been primarily interested in exporting Defs to RDF. However, we can also export instance data to RDF. In Haystack, an instance is a Dict with an [id](id) tag. The following outlines the high-level rules for exporting instances (i.e. Dicts) to RDF.
...
* Encode tag values as defined in the [data types](#datatypes) section.    + Exception: If the tag is a Ref (e.g. `siteRef`) the value should be a blank node with a label that references the corresponding instance.
